	...
	Themes = {
		HoneyHub = {
			["Color Hub 1"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 15, 32)),
				ColorSequenceKeypoint.new(0.40, Color3.fromRGB(15, 25, 55)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(8, 20, 45))
			}),
			["Color Hub 2"] = Color3.fromRGB(15, 22, 48),
			["Color Stroke"] = Color3.fromRGB(40, 65, 120),
			["Color Theme"] = Color3.fromRGB(0, 180, 255),
			["Color Text"] = Color3.fromRGB(255, 255, 255),
			["Color Dark Text"] = Color3.fromRGB(140, 170, 220),
			["Color Toggle On"] = Color3.fromRGB(0, 200, 255),
			["Color Toggle Off"] = Color3.fromRGB(30, 45, 80)
		},
		HoneyHubLight = {
			["Color Hub 1"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(230, 242, 255)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(245, 250, 255)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(210, 232, 255))
			}),
			["Color Hub 2"] = Color3.fromRGB(240, 248, 255),
			["Color Stroke"] = Color3.fromRGB(100, 160, 240),
			["Color Theme"] = Color3.fromRGB(0, 140, 255),
			["Color Text"] = Color3.fromRGB(15, 30, 60),
			["Color Dark Text"] = Color3.fromRGB(70, 100, 150),
			["Color Toggle On"] = Color3.fromRGB(0, 160, 255),
			["Color Toggle Off"] = Color3.fromRGB(180, 205, 235)
		},
		Wicked = {
			["Color Hub 1"] = ColorSequence.new({
...
AddEle("Button", function(parent, props, ...)
	...
	local BtnStroke = Make("Stroke", New)
	New.MouseEnter:Connect(function()
		CreateTween({New, "BackgroundTransparency", 0.25, 0.2})
		CreateTween({BtnStroke, "Color", Theme["Color Theme"], 0.2})
	end)
	New.MouseLeave:Connect(function()
		CreateTween({New, "BackgroundTransparency", 0, 0.2})
		CreateTween({BtnStroke, "Color", Theme["Color Stroke"], 0.2})
	end)
	if args[1] then
...
function redzlib:SetTheme(NewTheme)
	...
	SaveJson("honey_hub.json", redzlib.Save)
...
function redzlib:SetBackgroundImage(imageId)
    local success, err = pcall(function()
        local mainFrame = ScreenGui:FindFirstChild("Hub")
        if mainFrame then
            mainFrame.Image = "rbxassetid://" .. tostring(imageId)
            mainFrame.ImageTransparency = 0 
            mainFrame.ScaleType = Enum.ScaleType.Stretch 
        end
    end)
    if not success then
        warn("Failed to set background image:", err)
    end
end
...
local function CreateBlockScreen()
	local BlockFrame = Create("Frame", ScreenGui, {
		Size = UDim2.fromScale(1, 1),
	...
	InsertTheme(Create("UIGradient", BlockFrame, {
	...
	local AlertBox = Create("Frame", BlockFrame, {
		Size = UDim2.fromOffset(380, 220),
		Position = UDim2.fromScale(0.5, 0.5),
		AnchorPoint = Vector2.new(0.5, 0.5),
		BackgroundColor3 = Color3.fromRGB(15, 22, 48),
	})
	Make("Corner", AlertBox, UDim.new(0, 8))
	Make("Stroke", AlertBox, {}, Color3.fromRGB(255, 60, 60))
	
	local AlertIcon = Create("ImageLabel", AlertBox, {
	...
	local AlertTitle = Create("TextLabel", AlertBox, {
		Font = Enum.Font.GothamBold,
		Size = UDim2.new(1, -40, 0, 45),
		Position = UDim2.new(0.5, 0, 0, 85),
		AnchorPoint = Vector2.new(0.5, 0),
		Text = "Talvez vc executou uma versao falsa do honey hub",
	...
	local AlertMessage = Create("TextLabel", AlertBox, {
		Font = Enum.Font.Gotham,
		Size = UDim2.new(1, -40, 0, 45),
		Position = UDim2.new(0.5, 0, 0, 135),
		AnchorPoint = Vector2.new(0.5, 0),
		Text = "Esta copia da interface nao esta licenciada. Use o script oficial.",
	...
	local Footer = Create("TextLabel", AlertBox, {
		Font = Enum.Font.Gotham,
		Size = UDim2.new(1, -40, 0, 20),
		Position = UDim2.new(0.5, 0, 1, -15),
		AnchorPoint = Vector2.new(0.5, 1),
		Text = "Honey Hub Security Verification System",
		TextColor3 = Color3.fromRGB(80, 100, 140),
		TextSize = 9,
		BackgroundTransparency = 1
	})
end
...
local FakeObject = {}
FakeObject.__index = function(self, key)
	return function()
		return FakeObject
	end
end
local FakeWindow = setmetatable({}, FakeObject)

function redzlib:MakeWindow(Configs)
	local UserKey = Configs.Key or Configs.Password or Configs.Senha
	if UserKey ~= "HONEY-8C2D9-4B5E7-6F1A3" then
		CreateBlockScreen()
		return FakeWindow
	end
	
	local WTitle = Configs[1] or Configs.Name or Configs.Title or "Honey Hub"
	local WMiniText = Configs[2] or Configs.SubTitle or "by Bruno"
	...
	MainFrame.Size = UDim2.fromOffset(UISizeX * 0.8, UISizeY * 0.8)
	CreateTween({MainFrame, "Size", UDim2.fromOffset(UISizeX, UISizeY), 0.5})
	...
	local HorizontalSeparator = InsertTheme(Create("Frame", Components, {
		...
		Create("UIGradient", {
			Transparency = NumberSequence.new({
				...
				NumberSequenceKeypoint.new(0.10, 0.30),
				NumberSequenceKeypoint.new(0.90, 0.30),
	...
	local VerticalSeparator = InsertTheme(Create("Frame", Components, {
		Size = UDim2.new(0, 1, 1, -28),
		Position = UDim2.new(0, redzlib.Save.TabSize, 0, 28),
		BackgroundColor3 = Theme["Color Stroke"],
		BorderSizePixel = 0,
	...
	local function ControlSize()
		...
		VerticalSeparator.Position = UDim2.new(0, ControlSize2.Position.X.Offset, 0, TopBar.Size.Y.Offset)
	...
	function Window:CloseBtn()
		local Dialog = Window:Dialog({
			Title = "Fechar",
			Text = "Tem certeza que deseja fechar o Honey Hub?",
			Options = {
				{"Confirmar", function()
					ScreenGui:Destroy()
				end},
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddSection(Configs)
			...
			local LeftLine = InsertTheme(Create("Frame", SectionFrame, {
			...
			local RightLine = InsertTheme(Create("Frame", SectionFrame, {
		...
		function Tab:AddButton(Configs)
			...
			FButton.Activated:Connect(function()
				CreateTween({ButtonIcon, "Rotation", 90, 0.15})
				task.delay(0.15, function()
					CreateTween({ButtonIcon, "Rotation", 0, 0.15})
				end)
	...
	local ToggleHolder = Create("Frame", Button, {
		...
		Position = UDim2.new(1, -10, 0.5),
		AnchorPoint = Vector2.new(1, 0.5),
		BackgroundColor3 = Default and Theme["Color Toggle On"] or Theme["Color Toggle Off"],
		BackgroundTransparency = Default and 0.4 or 0
	})
	...
	local function SetToggle(Val)
		...
		if Default then
			...
			CreateTween({ToggleHolder, "BackgroundTransparency", 0.4, 0.25})
		else
			...
			CreateTween({ToggleHolder, "BackgroundTransparency", 0, 0.25})
		end
	...
	return Toggle
		...
		local ActiveLabel = InsertTheme(Create("TextLabel", SelectedFrame, {
			...
			TextColor3 = Theme["Color Text"],
			Text = "..."
		}), "Text")			local Arrow = Create("ImageLabel", SelectedFrame, {
